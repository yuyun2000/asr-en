...
def model(input_shape, dropout_rate=0.5):
    ...
    x = conv_block(inputs, 48, 1, strides=(2, 2))
    x = conv_block(x, 64, 1)
    x = conv_block(x, 96, 1, strides=(2, 2))
    x = depthwise_conv_block(x, 96, 1, 1, (1, 1))
    x = depthwise_conv_block(x, 96, 1, 1, (1, 1))
    # x = conv_block_withoutrelu(x, 64, strides=(1, 1))
    x = depthwise_conv_block(x,160,1,1,(2,2))
    x = depthwise_conv_block(x, 160, 1, 1, (1, 1))
    x = depthwise_conv_block(x, 160, 1, 1, (1, 1))
    x = depthwise_conv_block(x, 160, 1, 1, (1, 1))
    # x = depthwise_conv_block(x, 128, 1, 1, (1, 1))

    x = depthwise_conv_block(x, 256, 1, 1, (1, 1))
    x = depthwise_conv_block(x, 320, 1, 1, (1, 1))

    x = layers.Reshape(target_shape=(-1, 3200))(x)

    x = layers.Dropout(rate=dropout_rate)(x)

    x = layers.Dense(500,use_bias=True)(x)
...
if __name__ == '__main__':
    # 获得模型结构
    model = model(input_shape=[1600, 80, 1])  # 随即杀死神经元的概率
